        ...
        <Card>
            <CardHeader>
                <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-2xl text-primary font-bold">{t('weather.ai_recommendations')}</CardTitle>
                </div>
            </CardHeader>
            <CardContent>
                {isLoading ? <RecommendationSkeleton /> :
                    !recommendations || recommendations.recommendations.length === 0 ? (
                        <p className="text-muted-foreground">{t('weather.no_recommendations')}</p>
                    ) : (
                        <Accordion type="single" collapsible className="w-full" defaultValue="item-0">
                            {recommendations.recommendations.map((rec, index) => (
                                <AccordionItem value={`item-${index}`} key={index}>
                                    <AccordionTrigger className="font-semibold text-lg hover:no-underline">
                                        {t('weather.recommendation_for_day', index + 1)}
                                    </AccordionTrigger>
                                    <AccordionContent>
                                        <div className="space-y-4 p-2">
                                            <div className="flex items-start gap-4">
                                                ...
                                                <div>
                                                    <h4 className="font-semibold text-foreground">{t('weather.activity')}</h4>
                                            ...
                                            <div className="flex items-start gap-4">
                                                ...
                                                <div>
                                                    <h4 className="font-semibold text-foreground">{t('weather.clothing')}</h4>
                                                    <p className="text-muted-foreground">{rec.clothing}</p>
                                                </div>
                                            </div>
                                            <div>
                                                <h4 className="font-semibold text-foreground mb-1">{t('weather.justification')}</h4>
